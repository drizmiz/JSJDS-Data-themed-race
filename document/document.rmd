...
“一带一路”是我国的重要战略之一. 由于涉及国家广泛，文化背景复杂，每个国家的实际情况不同，我们需要通过数据评价一带一路对其沿线国家的影响. 为此，我们使用双重差分法、合成控制法对政策效果进行反事实推断，试图评估其置信区间和P值. 此外，我们进行了一些可视化工作以直观反映其对不同地理位置的国家的影响. 对于投资水平，由于其缺失数据较多，我们进行了缺失数据填补. 此后，我们发现，一带一路提出后，中国与一些沿线国家的投资水平有了显著额外增加. 而对多数国家，中国对其投资水平并未如愿得到额外增长. 同时，我们将一带一路沿线国家与中国的相互投资水平进行了可视化，可以直观反映出一带一路的成效，并对不同国家进行定性比较. 对于医疗健康状况，我们利用世界健康数据集，研究了一带一路对沿线国家健康水平的影响. 数据结果表明，一带一路在一定程度上促进了沿线国家健康水平的提升.
...
根据 _R for Data Science_[@Wickham2017R]，数据分析的一般流程如[图1](#wf)所示. 本项目也符合这种模式.

![The Data Science Workflow[^1]](resources/workflow.png){#wf}

[^1]: 此图取自 _R for Data Science_，以[CC BY-NC-ND 3.0 US](http://creativecommons.org/licenses/by-nc-nd/3.0/us/) 发布.
...
虽然不为人所重视，数据清洗往往是数据分析工作中耗时最久的部分.[@DS50] 不仅如此，数据清洗的目标也往往不被人熟知. 什么数据结构才算是整理好？幸运的是，Hadley Wickham的文章[@tidy-data]向我们提供了一种通俗易懂的解释：每行应该代表一个观察(observation)，每列应该代表一个变量(variable). 根据此种原则，并利用其为此目的开发的R包**tidyverse**[@tdv]，进行数据清洗. 

## 数据分析

数据建模和分析是传统上受重视的技术. 以下对各个国家分别进行的分析结果存储于本项目`results`文件夹下的`results_IMR`（婴儿死亡率数据）和`results_invest`（投资数据）文件夹中. 其中，`sens.csv`描述了对测试进行安慰剂检验(placebo test)的情况，`p.noeff.csv`描述了测试的P值，`ci.csv`描述了结果的置信区间，`pdf`文件绘制出了按国家分类的置信区间的情况.

首先注意到数据集中存在许多缺失数据，如[图2](#missd)[@citation_VIM]所示. 缺失数据的删除或填补需要用到一些技术.

![缺失数据示意图](resources/Missing data.pdf){#missd .class width=75% height=75%}
...
分析一带一路的影响，就是要分析某一事件发生后，某一值的变化情况，这是经典的政策影响问题. 而该值自身必然存在着模型外因素导致的变化趋势，所以我们需要利用某些技术来去除这些因素的影响. 于是，我们运用双重差分和合成控制两种技术. 参见[方法$\star$](#method). 例如，黑山的新生儿死亡率随时间的反事实变化绘图如下.

![黑山](resources/黑山_ci_sc.pdf){#hs .class width=50% height=50%}
...
![一带一路对五岁以下儿童死亡率的总效应示意图](resources/5MR_pooled.pdf){#pool .class width=50% height=40%}
...
此外，我们将两种方法生成的置信区间的均值绘图，分别按照时间组织为Timeline排列在Page中，例如[世界健康-ci.html](../visualization/out/世界健康-ci.html). 粗略地我们可以看到，一些特定的年份中，自我国西部延伸至西亚，较高的分层设色部分都分布在一带一路沿线。
...
且平均而言，参与一带一路反而额外降低中国对当地的投资的对数约0.40.

根据常识，投资($Y$)对经济发展水平($Z$)的平均因果作用是正的，而经济发展水平($Z$)对健康水平($W$)的平均因果作用也是正的. 我们建立以下模型.

![数据模型示意图](resources/DAG.pdf){#dag .class width=60% height=60%}

这样，根据我们的模型，在相同的经济发展水平下，参与一带一路能够相比预期增加沿线国家的国民健康水平，或者狭义来说，降低婴儿死亡率. 从这一方面来看，一带一路对沿线国家经济以外的影响，还有待更深的评估.

## 不足和展望

我们的报告最显著的不足之处是将不同国家的结论组合的方式显得过于随意，仅仅是计算每个国家的效应然后求平均，没有考虑到国家和国家的权重问题. 类似地，在计算投资额时，可以考虑除去国家的经济总量，以反映中国投资占比占该国家GDP的比例.

# 方法$\star$ {#method}

## 数据补全

我们调用R包**mice**[@citation_mice]，采用linear regression with bootstrap的方法进行缺失数据填补. 首先，对数据进行bootstrap重抽样[@citation_bs]，然后进行线性回归插值，最终计算得到合成的结果. 其中，多元线性回归采用Schafer的算法[@schafer_1997].
...
我们利用Chernozhukov et al.[@doi:10.1080/01621459.2021.1920957]的方法分析P值和置信区间等信息.

### 有向无环图

本文的有向无环图，在R语言中用**ggdag**[@citation_ggdag]生成. 这是有向无环图(Directed acyclic graph, DAG)，其中边代表因果作用[@pearl]. 注意到其中不存在 $X\rightarrow Z$的线，即 $X\mathrel{\perp\!\!\!\perp}Z|Y$. 这是假定一带一路不会以其他方式影响地区的经济发展水平（即不存在$X\rightarrow Z$的线），以避免出现替代指标悖论.
...
其中，数据可视化相关的脚本以及输出结果全部储存在`./visualization`中：
...
以FDI（对外直接投资）为例，我们讲述项目中使用的pyecharts可视化方法如下.
